...
class WarehousesTab(QWidget):
    def __init__(self):
        ...
        self.search_input.setPlaceholderText("Поиск по адресу или типу")
    ...
    def fill_table(self, rows):
        self.current_ids = [row["warehouse_id"] for row in rows]
        self.table.setRowCount(len(rows))
        for i, row in enumerate(rows):
            self.table.setItem(i, 0, QTableWidgetItem(row.get("warehouse_address", "")))
            self.table.setItem(i, 1, QTableWidgetItem(str(row.get("capacity", ""))))
            self.table.setItem(i, 2, QTableWidgetItem(row.get("type", "")))
    ...
    def delete(self):
        ...
        if not warehouse_id:
            QMessageBox.warning(self, "Ошибка", "Выберите склад")
            return
        reply = QMessageBox.question(self, "Подтверждение", "Удалить склад?",
    ...
    def export_pdf(self):
        path, _ = QFileDialog.getSaveFileName(self, "PDF — Запасы", "Запасы.pdf", "PDF Files (*.pdf)")
        if not path:
            return
        try:
            ReportService().stock_report(path)
